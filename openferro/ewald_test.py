...
def dipole_dipole_ewald(field, parameters, alpha=1):
    """
    ...
    G_grid = jnp.stack( jnp.meshgrid(
        (jnp.arange(l1)-(l1//2)) / l1 * b[0], 
        (jnp.arange(l2)-(l2//2)) / l2 * b[1], 
        (jnp.arange(l3)-(l3//2)) / l3 * b[2], 
        indexing='ij'), axis=-1)   # (l1, l2, l3, 3)
    G_grid = jnp.roll(G_grid, shift=(-(l1//2), -(l2//2), -(l3//2)), axis=(0,1,2))
    ...
    Uk_squared = jnp.sum(F_fft3.real * G_grid, axis=-1)**2
    Uk_squared += jnp.sum( F_fft3.imag * G_grid , axis=-1)**2   # (l1, l2, l3)
    ...
    # print('ksum:',ewald_ksum, 'rsum:', ewald_rsum)
    return ewald_ksum + ewald_rsum

def dipole_dipole_ewald_slow(field, latt_vec, alpha = 1):
    """
    ...
    if (a1[1] != 0) or (a1[2] != 0) or (a2[0] != 0) or (a2[2] != 0) or (a3[0] != 0) or (a3[1] != 0):
        ...
        bmax = jnp.max(b)
        amin = 2 * np.pi / bmax
    ref_volume = a[0] * a[1] * a[2] * l1 * l2 * l3
    # tol = 1.0e-12
    # alpha = jnp.sqrt(-jnp.log(tol)) / amin
    sigma = 1.0 / alpha / jnp.sqrt(2.0)   ## the ewald sigma parameter
    ## get k-grid
    ...
    gamma_loc = (l1//2) * (l2*l3) + (l2//2) * l3 + (l3//2)
                        ...
                        for j3 in range(l3):
                            for alpha in range(3):
                                for beta in range(3):
                                    ...
                                    Q_ijabk = Q_ijabk.at[gamma_loc].set(0.0)   # mute Gamma point
...
if __name__ == "__main__":
    ...
    for alpha in [   2,  ]: #np.linspace(2, 20, 10):
        E1 = dipole_dipole_ewald(field,  paras, alpha=alpha)
        # E2 = dipole_dipole_ewald_slow(field, latt_vec, alpha=alpha)
        force_func = grad(dipole_dipole_ewald)
        force = force_func(field, paras, alpha=alpha) 
        print('ewald energy:', E1)
        print('ewald force:', force[0,0,0])
