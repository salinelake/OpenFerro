...
def dipole_dipole_ewald(field, parameters):
    ...
    n1 = jnp.array(gcut / b[0]).astype(int)
    n2 = jnp.array(gcut / b[1]).astype(int)
    n3 = jnp.array(gcut / b[2]).astype(int)
...
if __name__ == "__main__":
    ...
    l1,l2,l3 = 2,2,2
    a1 = jnp.array([1.0, 0.0, 0.0]) * 4
    a2 = jnp.array([0.0, 1.0, 0.0]) * 4
    a3 = jnp.array([0.0, 0.0, 1.0]) * 4
    latt = BravaisLattice3D(l1, l2, l3, a1, a2, a3)
    ...
    E1 = dipole_dipole_ewald(field,  paras)
    E2 = dipole_dipole_ewald_slow(field, paras)
    force_func =  grad(jit(dipole_dipole_ewald)) 
    force = force_func(field, paras ) 
    print('ewald energy:', E1)
    print('ewald energy (direct summation):', E2)
